    ...
    async presencesUpdate(client) {
        ...
        if (
            ...
            for (const [ memberId, presence ] of listedPresences) {     
                        ...
                        .then(() => {
                            channel.send({
                                embeds: [
                                    ...
                                    .setDescription(`• <@&1357161320679477459> rolü, ${member.displayName}'nın/nin durum mesajından \`${text}\` ifadesini kaldırdığı için kendisinden **alındı**.`)
                ...
                if (!hasRole && matchFound) { // Eğer rol yoksa ve eşleşme bulunursa
                        ...
                        .then(() => {
                            channel.send({
                                embeds: [
                                    ...
                                    .setAuthor({ name: member.user.username, iconURL: member.user.displayAvatarURL() })    
                                    .setDescription(`• <@&1357161320679477459> rolü, ${member.displayName}'nın/nin durum mesajını \`${text}\` yapması nedeniyle kendisine **verildi**.`)
                                    .setThumbnail(member.user.displayAvatarURL())
                                    .setFields([
                                        {
                                            name: 'Kullanıcı etiketi:',
