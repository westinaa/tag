                ...
                if (hasRole && !matchFound) {
                    ...
                    if (hasColorRole && !isBooster) {
                        ...
                        for (const [id] of removableColors) {
                            await member.roles.remove(id, 'Renk rolü de kaldırıldı.').catch(() => null);
                        }
                    }
                    ...
                    await channel.send({
                        embeds: [
                            new EmbedBuilder()
                                ...
                                .setDescription(`• <@&${roleId}> rolü, ${member.displayName}'nın/nin durum mesajından \`${text}\` ifadesini kaldırdığı için kendisinden **alındı**.`)
